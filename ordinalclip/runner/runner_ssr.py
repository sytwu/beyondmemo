...
class Runner(pl.LightningModule):
    ...
    # Running Steps
    def run_step(self, batch, batch_idx, run_type=None):
        x, labels = batch
        year = labels[0]
        
        if not isinstance(year, torch.Tensor):
            year = torch.tensor(np.array(year).astype(int), device=device)

        show_importance = (run_type=="test")
        logits, regression_age, *_ = self.module(x, labels[1:] if len(labels)>1 else None, run_type)

        losses = self.compute_losses(logits, regression_age, year)
        loss = sum([weight * losses[k] for k, weight in self.loss_weights.items()])

        metrics_exp = self.compute_per_example_metrics(logits, regression_age, year, "exp")
        metrics_max = self.compute_per_example_metrics(logits, regression_age, year, "max")

        # losses = self.compute_losses(logits, year)
        # loss = sum([weight * losses[k] for k, weight in self.loss_weights.items()])

        # metrics_exp = self.compute_per_example_metrics(logits, year, "exp")
        # metrics_max = self.compute_per_example_metrics(logits, year, "max")
    ...
    def test_step(self, batch, batch_idx):
        outputs = self.run_step(batch, batch_idx, run_type="test")

        return outputs

    # Epoch Eval
    def eval_epoch_end(self, outputs, run_type=None):
        """_summary_
    ...
    def test_epoch_end(self, outputs) -> None:
        ...
        if preds_list:
            ...
            import pandas as pd
            csv_path = self.output_dir / f"{self.csv_prefix}_test.csv"
            df = pd.DataFrame({"predict_y": predictions.flatten()})
            df.to_csv(csv_path, index=False)
            self._custom_logger.info(f"Saved test predictions to {csv_path}")
    ...
    def logging(self, outputs: dict, run_type: str, on_step=True, on_epoch=True, batch_size=None):
        for k, v in outputs.items():
            if self._valid_key(k):
                self.log(f"{run_type}_{k}", v.mean(), on_step=on_step, on_epoch=on_epoch, prog_bar=False, logger=True, batch_size=batch_size)
    ...
    def compute_losses(self, logits, regression_age, y):
        ...
        losses["ce_loss"] = self.compute_ce_dis_loss(logits, y_label,7)
        losses["kl_loss"] = self.compute_kl_dis_loss(logits, y_label,7)
    ...
    def compute_kl_dis_loss(self, logits, y, d):
        ...
        y_column = y.permute(*torch.arange(y.ndim - 1, -1, -1))
        ...
        for i in range(logits_t.shape[0]):
            if y_t_row_ind[i] > 0:
                ...
                label_inv_ranks = label_inv_ranks # * label_inv_ranks # dij = (yi-yj)^2
                label_inv_ranks_norm = label_inv_ranks / torch.sum(label_inv_ranks,dim=1) * (d-1)
                ...
                label_inv_ranks_norm = torch.ones(logits_t.shape[1]).to(device)
                ls_label_inv_ranks_norm = label_inv_ranks_norm.detach().cpu().numpy().tolist()
            
            ls_weight.append(ls_label_inv_ranks_norm)

        weight = torch.Tensor(ls_weight).to(device)
    ...
    def compute_ce_dis_loss(self,logits,y,d):
        ...
        target = torch.Tensor(list_target).to(device)
    ...
    def num2label(self, y):
        ...
        for i in range(len(y_num)):
            if 800<= y_num[i] <=1150:
                y_label[i] = 0
            elif 1150<= y_num[i] <=1400:
                y_label[i] = 1
            elif 1400<= y_num[i] <=1600:
                y_label[i] = 2
            elif 1600<= y_num[i] <=1800:
                y_label[i] = 3
            elif 1800<= y_num[i] <=1900:
                y_label[i] = 4
            elif 1900<= y_num[i] <=1950:
                y_label[i] = 5
            else:
                y_label[i] = 6
    ...
    def compute_per_example_metrics(self, logits, regression_age, y, gather_type="exp"):
        ...
        if gather_type == "exp":
            ...
            return {f"mae_{gather_type}_metric": mae, f"acc_{gather_type}_metric": acc}
    ...
    def load_weights(
        ...
        init_location_encoder_weights=None,
        ...
        if init_location_encoder_weights is not None:
            self._custom_logger.info("init_location_encoder_weights")
            load_pretrained_weights(self.module.location_encoder, init_location_encoder_weights)
    ...
    def build_param_dict(
        ...
        if lr_image_adapter > 0 and self.module.zero_conv is not None:
            param_dict_ls.append(
                {
                    "params": self.module.zero_conv.parameters(),
                    "lr": lr_image_adapter,
            ...
            self._custom_logger.info("freeze_param(self.model.zero_conv)")
            freeze_param(self.module.zero_conv)
